...
var GH = 'https://github.com/urlynn';
var GH_PSLINKB = GH + '/PSLinkB';
var GH_PSLINKB_VER = GH_PSLINKB + '/blob/main/luci-app-pslinkb/VERSION';
var GH_PSLINKB_REL = GH_PSLINKB + '/releases/latest';
...
var callVersionLatest = rpc.declare({
	object: 'luci.pslinkb',
	method: 'version_latest_get'
});

var callAppVersion = rpc.declare({
	object: 'luci.pslinkb',
	method: 'app_version_get'
});

var callLuciVersionLatest = rpc.declare({
	object: 'luci.pslinkb',
	method: 'luci_version_latest_get'
});

var callCheckBinary = rpc.declare({
	object: 'luci.pslinkb',
	method: 'check_binary'
});

var callInstallBinary = rpc.declare({
	object: 'luci.pslinkb',
	method: 'install_binary'
});
...
return view.extend({
	load: function() {
		...
		return Promise.all([
			...
			callVersion().catch(function() { return { version: '' }; }),
			callAppVersion().catch(function() { return { version: '' }; })
		]).then(function(results) {
			window._pslinkbInit = {
				svc: results[0],
				dns: results[1],
				ver: results[2].version,
				appVer: results[3].version
			};
		});
	},
	...
	addFooter: function() {
		...
		var verTxt = d2.ver ? 'PSLinkB v' + d2.ver : 'PSLinkB N/A';
		var appTxt = 'Luci v' + (d2.appVer || '?');
		...
		if (links.length > 1) {
			// 多链接（Bootstrap/Argon）：仅操作与第一条链接同级的 <a>
			var parent = links[0].parentNode;
			var siblings = parent.querySelectorAll('a');
			var items = [
				{ href: GH, text: '© 2026 Urlynn' },
				{ href: GH_PSLINKB_REL, text: verTxt },
				{ href: GH_PSLINKB_VER, text: appTxt }
			];
			...
			// 单气泡（Alpha）：仅一个链接，· 分隔
			footer.innerHTML =
				'<a href="' + GH + '" target="_blank">&copy; 2026 Urlynn \u00B7 ' + verTxt + ' \u00B7 ' + appTxt + '</a>';
		} else {
			footer.innerHTML =
				'<a href="' + GH + '" target="_blank">&copy; 2026 Urlynn</a>' +
				' <span style="opacity:0.3">\u00B7</span> ' +
				'<a href="' + GH_PSLINKB_REL + '" target="_blank">' + verTxt + '</a>' +
				' <span style="opacity:0.3">\u00B7</span> ' +
				'<a href="' + GH_PSLINKB_VER + '" target="_blank">' + appTxt + '</a>';
		}
		...
		// 阶段1：tabmenu 就绪后立刻显示本地版本（来自 load）
		var d2 = window._pslinkbInit || {};
		var _inject = function() {
			...
			self._renderVer(li, d2.ver || '', d2.appVer || '', '', '');
			// 阶段2：远程版本（sessionStorage 缓存，一次会话仅拉一次）
			var cached = sessionStorage.getItem('_pslinkbVer');
			if (cached) {
				var v = JSON.parse(cached);
				self._renderVer(li, d2.ver || '', d2.appVer || '', v.verLatest || '', v.luciLatest || '');
			} else {
				Promise.all([
					callVersionLatest().catch(function() { return { version: '' }; }),
					callLuciVersionLatest().catch(function() { return { version: '' }; })
				]).then(function(v) {
					var data = { verLatest: v[0].version || '', luciLatest: v[1].version || '' };
					sessionStorage.setItem('_pslinkbVer', JSON.stringify(data));
					self._renderVer(li, d2.ver || '', d2.appVer || '', data.verLatest, data.luciLatest);
				});
			}
			return true;
		};
		...
		// pslinkb 未安装检测
		callCheckBinary().then(function(r) {
			...
			modal.innerHTML = '<div class="cbi-section ps-dialog" style="padding:24px;max-width:400px;text-align:center;margin:0">'
				+ '<h3 style="margin:0 0 12px">PSLinkB ' + _('not installed') + '</h3>'
				+ '<p style="margin:0 0 20px">' + _('PSLinkB binary not found. Install now?') + '</p>'
				...
				+ '</div>'
				+ '<div class="cbi-section ps-dialog js-install-log-win" style="padding:16px;max-width:400px;text-align:left;margin:0;display:none">'
				...
				+ '<span style="font-size:18px;line-height:1;cursor:pointer;opacity:0.5" onclick="this.closest(\'.js-install-log-win\').style.display=\'none\'">&times;</span>'
				+ '</div>'
			...
			var logWin = modal.querySelector('.js-install-log-win');
			...
			installBtn.addEventListener('click', function() {
				...
				callInstallBinary().then(function(res) {
					if (res && res.ok) {
						...
						msgEl.textContent = (res && res.error) ? res.error : _('Installation failed');
						if (res && res.log) {
					...
					cancelBtn.disabled = false;
					installBtn.textContent = _('Install');
					msgEl.textContent = _('Installation failed');
				});
			});
		});
	...
	_renderVer: function(li, ver, appVer, verLatest, luciLatest) {
		li.innerHTML = '';
		if (ver) {
			var pNewer = verLatest && this._verNewer(verLatest, ver);
			var a = document.createElement('a');
			a.href = GH_PSLINKB_REL;
			a.target = '_blank';
			a.appendChild(document.createTextNode('v' + (pNewer ? verLatest : ver)));
			if (pNewer) {
		...
		if (appVer) {
			...
			var lNewer = luciLatest && this._verNewer(luciLatest, appVer);
			var b = document.createElement('a');
			b.href = GH_PSLINKB_VER;
			b.target = '_blank';
			b.appendChild(document.createTextNode('Luci v' + (lNewer ? luciLatest : appVer)));
			if (lNewer) {
	...
	_updateDns: function(d) {
		...
		if (sw) {
			...
			if (!d.checking) sw.disabled = false;
		}
